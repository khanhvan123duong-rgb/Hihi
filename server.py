...
import json, os, random, string, time, hashlib, threading, shutil, urllib.request as _ureq, urllib.parse, math
...
def before():
    if request.method == 'OPTIONS':
        return
    path = request.path
    if any(path.endswith(ext) for ext in ['.jpg', '.jpeg', '.png', '.gif', '.mp3', '.mp4', '.webm', '.ico', '.css', '.js']):
        return
    ...
    if not ok:
        return make_response(f'<h1>429 Too Many Requests</h1><p>IP bị tạm khóa {wait}s. Vui lòng thử lại sau.</p>', 429)
    uid = session.get('uid')
    if uid:
        db = load_db()
        u = db['users'].get(uid)
        if not u or u.get('pw') != session.get('spw'):
            session.clear()
...
def api_buy():
    ...
    for a in bought:
        for i, acc in enumerate(db['accounts'][cat]):
            if acc.get('id') == a.get('id') and not acc.get('sold'):
                db['accounts'][cat][i].update({'sold': True, 'sold_to': uid, 'sold_time': now_vn()})
    ...
    db['orders'].append({
        ...
        'cat': cat, 'qty': qty, 'total': total, 'accs': result_accs, 'time': now_vn()
    })
...
let aud=null,curT=0,isPlaying=false;
function initAudio(){
  if(aud)return;
  aud=new Audio();
...
function playM(){if(!aud)initAudio();aud.play().catch(()=>{});isPlaying=true;updatePlayBtn();const d=document.getElementById('mdisc');if(d)d.classList.add('playing');}
...
<div class="page active visible" id="pg-home">
  ...
  <div class="card" style="margin-bottom:.9rem;">
    ...
    <div class="rainbow-wrap" style="margin-bottom:.7rem;cursor:pointer;" onclick="showPage('shop',document.getElementById('mi-shop'))">
      <img src="/acc_kim_cuong.jpg" alt="Acc Kim Cương" style="width:100%;border-radius:11px;display:block;" loading="lazy">
    </div>
    <div class="rainbow-wrap" style="margin-bottom:.7rem;cursor:pointer;" onclick="showPage('shop',document.getElementById('mi-shop'))">
      <img src="/acc_bach_kim.png" alt="Acc Bạch Kim" style="width:100%;border-radius:11px;display:block;" loading="lazy">
    </div>
    <div class="rainbow-wrap" style="cursor:pointer;" onclick="showPage('shop',document.getElementById('mi-shop'))">
      <img src="/acc_lv5.jpg" alt="Acc Lv5 Google" style="width:100%;border-radius:11px;display:block;" loading="lazy">
    </div>
  </div>

  <div class="card" style="margin-bottom:.9rem;">
    <div class="card-title">📦 Kho Acc Hiện Tại</div>
    <div style="display:flex;flex-direction:column;gap:.5rem;">
      <div style="display:flex;justify-content:space-between;align-items:center;padding:.45rem 0;border-bottom:1px solid var(--border);">
        <div><div style="font-weight:600;font-size:.85rem;">💜 Clon Kim Cương</div><div style="font-size:.72rem;color:var(--muted);">20.000đ/acc</div></div>
        <div style="font-weight:700;font-size:.82rem;" id="hm-cnt-kim_cuong" class="acc-stock">---</div>
      </div>
      <div style="display:flex;justify-content:space-between;align-items:center;padding:.45rem 0;border-bottom:1px solid var(--border);">
        <div><div style="font-weight:600;font-size:.85rem;">🔵 Clon Bạch Kim</div><div style="font-size:.72rem;color:var(--muted);">15.000đ/acc</div></div>
        <div style="font-weight:700;font-size:.82rem;" id="hm-cnt-bach_kim" class="acc-stock">---</div>
      </div>
      <div style="display:flex;justify-content:space-between;align-items:center;padding:.45rem 0;">
        <div><div style="font-weight:600;font-size:.85rem;">🟢 Clon Lv5 Google</div><div style="font-size:.72rem;color:var(--muted);">2.500đ/acc</div></div>
        <div style="font-weight:700;font-size:.82rem;" id="hm-cnt-lv5" class="acc-stock">---</div>
      </div>
    </div>
  </div>

  <!-- FEEDBACK SECTION — chỉ hiện khi có bài -->
...
<div class="page" id="pg-shop">
  ...
  <div id="sh-kim_cuong">
    <div class="acc-card">
      <div class="rainbow-wrap">
        <img src="/acc_kim_cuong.jpg" alt="Kim Cương" style="width:100%;display:block;" loading="lazy" onerror="this.parentNode.style.cssText='background:linear-gradient(135deg,#8b5cf6,#7c3aed);border-radius:14px;height:170px;'">
      </div>
      <div class="acc-body">
  ...
  <div id="sh-bach_kim" style="display:none;">
    <div class="acc-card">
      <div class="rainbow-wrap">
        <img src="/acc_bach_kim.png" alt="Bạch Kim" style="width:100%;display:block;" loading="lazy" onerror="this.parentNode.style.cssText='background:linear-gradient(135deg,#0891b2,#0e7490);border-radius:14px;height:170px;'">
      </div>
      <div class="acc-body">
  ...
  <div id="sh-lv5" style="display:none;">
    <div class="acc-card">
      <div class="rainbow-wrap">
        <img src="/acc_lv5.jpg" alt="Lv5" style="width:100%;display:block;" loading="lazy" onerror="this.parentNode.style.cssText='background:linear-gradient(135deg,#2563eb,#1d4ed8);border-radius:14px;height:170px;'">
      </div>
      <div class="acc-body">
...
<div class="page" id="pg-carry">
  <div class="card" style="margin-bottom:.9rem;padding:0;overflow:hidden;">
    <div class="rainbow-wrap" style="cursor:pointer;" onclick="void(0)">
...
<div class="modal-ov" id="topup-modal">
  <div class="modal">
    ...
    <div style="font-size:.75rem;color:var(--muted);margin-bottom:.9rem;line-height:1.5;background:#fffbeb;border-radius:10px;padding:.6rem;border:1px solid #fde68a;">
      ⚠️ <b>Ghi đúng nội dung chuyển khoản phía trên.</b> Sau khi chuyển, admin sẽ duyệt trong vài phút. Sai nội dung sẽ không được duyệt!
    </div>
    <div style="display:flex;gap:.65rem;">
      ...
      <a id="topup-tg-link" href="#" target="_blank" style="flex:1;">
        <button class="btn btn-tg btn-full">📲 Báo Admin TG</button>
      </a>
    </div>
  </div>
</div>
...
<div class="modal-ov" id="carry-modal">
  <div class="modal" style="text-align:center;">
    <div class="st-check" style="margin:0 auto .75rem;display:flex;"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg></div>
    <div class="modal-title" style="margin-bottom:.3rem;">Đặt Thành Công! 🎉</div>
    <div style="font-size:.82rem;color:var(--muted);margin-bottom:1rem;" id="carry-success-msg">Đơn kéo thuê đã được ghi nhận. Liên hệ admin để bắt đầu kéo!</div>
    <div style="background:#f8f9fb;border-radius:12px;padding:.9rem;margin-bottom:1rem;text-align:left;">
      <div style="font-size:.78rem;font-weight:700;color:var(--muted);margin-bottom:.5rem;">📲 LIÊN HỆ ADMIN ĐỂ KÉO:</div>
      <a href="#" id="carry-tg-link" target="_blank" style="display:block;margin-bottom:.45rem;">
        <button class="btn btn-tg btn-full">✈️ Nhắn Telegram @""" + ADMIN_TG_USERNAME + """</button>
      </a>
      <a href="https://www.tiktok.com/@midu.c2" target="_blank" style="display:block;">
        <button class="btn btn-tt btn-full">🎵 TikTok @midu.c2</button>
      </a>
    </div>
    <button class="btn btn-outline btn-full" onclick="document.getElementById('carry-modal').classList.remove('show')">Đóng</button>
  </div>
</div>
...
let curCat='kim_cuong',stockMap={};
let _topupTimer=null;

function renderStock(){
  let totalStock=0;
  Object.entries(stockMap).forEach(([cat,cnt])=>{
    totalStock+=cnt;
    const els=[document.getElementById('stk-'+cat),document.getElementById('hm-cnt-'+cat)];
    els.forEach(el=>{if(!el)return;
      if(cnt===0){el.textContent='❌ Hết hàng';el.className='acc-stock s-empty';}
      else if(cnt<5){el.textContent='⚠️ Còn ít: '+cnt;el.className='acc-stock s-low';}
      else{el.textContent='✅ Còn: '+cnt;el.className='acc-stock s-ok';}
    });
  });
  const qs=document.getElementById('hm-stk-kc');
  if(qs){
    if(totalStock===0){qs.textContent='Hết hàng';qs.style.color='#ef4444';}
    else{qs.textContent=totalStock+' acc';qs.style.color='var(--accent)';}
  }
}
function shopTab(cat,el){
  ['kim_cuong','bach_kim','lv5'].forEach(c=>{
    ...
    if(c===cat){
      ...
      requestAnimationFrame(()=>{s.style.transition='opacity .2s,transform .2s';s.style.opacity='1';s.style.transform='translateY(0)';});
    } else {
...
.stats{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:.7rem;margin-bottom:1.25rem;}
...
.stat-val{font-size:1.5rem;font-weight:800;color:var(--acc);}
.stat-lbl{font-size:.72rem;color:var(--muted);margin-top:.15rem;font-weight:500;}
.tabs{display:flex;gap:.2rem;background:#f3f4f6;padding:.25rem;border-radius:11px;margin-bottom:1.1rem;flex-wrap:wrap;}
.atab{flex:1;min-width:70px;padding:.5rem .3rem;border-radius:8px;text-align:center;font-size:.73rem;font-weight:600;cursor:pointer;color:var(--muted);transition:.2s;white-space:nowrap;}
...
th{background:#f3f4f6;padding:.55rem .7rem;text-align:left;font-weight:600;color:var(--muted);font-size:.72rem;text-transform:uppercase;}
...
.log-event{font-weight:600;color:var(--acc);flex-shrink:0;}
...
<div class="content">
  ...
  <div class="tabs">
    ...
    <div class="atab" onclick="aTab('orders',this)">🛒 Đơn hàng</div>
    ...
    <div class="card">
      ...
      <div style="margin-bottom:1rem;">
        <div style="font-weight:700;font-size:.88rem;margin-bottom:.5rem;">
          ...
          {{ db.accounts[cat]|selectattr('sold','equalto',False)|list|length }}/{{ db.accounts[cat]|length }} còn
        </div>
        {% if db.accounts[cat] %}
        <table>
          <tr><th>STT</th><th>User</th><th>Pass</th><th>Platform</th><th>Trạng thái</th><th>Xóa</th></tr>
          {% for acc in db.accounts[cat][-20:] %}
          <tr>
            <td style="color:var(--muted);">{{ loop.index }}</td>
            <td style="font-family:monospace;font-size:.75rem;">{{ acc.user }}</td>
            <td style="font-family:monospace;font-size:.75rem;">{{ acc.pass }}</td>
            <td>{{ acc.get('platform','FB') }}</td>
            <td><span class="badge {{ 'badge-g' if not acc.get('sold') else 'badge-r' }}">{{ 'Còn' if not acc.get('sold') else 'Đã bán' }}</span></td>
            <td><button class="btn btn-r btn-sm" onclick="delAcc('{{ cat }}','{{ acc.id }}')">Xóa</button></td>
          </tr>
          {% endfor %}
        </table>
  ...
  <div class="panel" id="pn-orders">
    <div class="card">
      <h3>🛒 Đơn Hàng Gần Đây ({{ db.orders|length }})</h3>
      <table>
        <tr><th>Thời gian</th><th>User</th><th>Loại</th><th>SL</th><th>Tổng</th></tr>
        {% for o in db.orders[-50:]|reverse %}
        <tr>
          <td style="font-size:.74rem;color:var(--muted);">{{ o.time }}</td>
          <td><b>{{ o.username }}</b></td>
          <td>{{ o.cat }}</td>
          <td style="text-align:center;">{{ o.qty }}</td>
          <td style="color:var(--acc);font-weight:700;">{{ "{:,}".format(o.total) }}đ</td>
        </tr>
        {% endfor %}
      </table>
    </div>
  ...
  <div class="panel" id="pn-carry">
    <div class="card">
      <h3>🏆 Đơn Kéo Thuê ({{ db.get('carry_orders',[])|length }})</h3>
      <div style="font-size:.78rem;color:var(--muted);margin-bottom:.7rem;background:#fffbeb;padding:.5rem .7rem;border-radius:8px;border:1px solid #fde68a;">
        💡 Bảng giá: Đồng~Kim Cương = 1.000đ/sao | Cao Thủ~Thách Đấu = 1.500đ/sao | Tối thiểu 5 sao
      </div>
      <table>
        <tr><th>Thời gian</th><th>User</th><th>Sao</th><th>Rank</th><th>Đơn giá</th><th>Ghi chú</th><th>Tổng</th></tr>
        ...
        <tr>
          ...
          <td>{{ o.get('rank','') }}</td>
          <td style="font-size:.74rem;">{{ "{:,}".format(o.get('price_per_star',1000)) }}đ/⭐</td>
          <td style="font-size:.74rem;color:var(--muted);">{{ o.get('note','') }}</td>
          <td style="color:var(--acc);font-weight:700;">{{ "{:,}".format(o.total) }}đ</td>
        </tr>
        {% endfor %}
      </table>
...
function approveTopup(content,amount){
  if(!confirm(`Duyệt ${amount.toLocaleString?amount.toLocaleString('vi-VN'):amount}đ?`))return;
